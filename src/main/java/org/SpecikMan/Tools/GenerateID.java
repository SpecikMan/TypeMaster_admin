...
public class GenerateID {
    ...
    private static final RankingLevelDao rkDao = new RankingLevelDao();
    ...
    public static String genShop() {
        List<Shop> grs = shopDao.getAll();
        if (grs.isEmpty()) {
            return "IT1";
        } else {
            List<Integer> nums = new ArrayList<>();
            grs.forEach(element -> nums.add(Integer.valueOf(element.getIdItem().replaceAll("[^0-9]", ""))));
            Collections.sort(nums);
            return "IT"+(nums.get(nums.size()-1)+1);
        }
    }
    public static String genRankingLevel(){
        List<RankingLevel> grs = rkDao.getAll();
        if (grs.isEmpty()) {
            return "RK1";
        } else {
            List<Integer> nums = new ArrayList<>();
            grs.forEach(element -> nums.add(Integer.valueOf(element.getIdRankingLevel().replaceAll("[^0-9]", ""))));
            Collections.sort(nums);
            return "RK"+(nums.get(nums.size()-1)+1);
        }
    }
}